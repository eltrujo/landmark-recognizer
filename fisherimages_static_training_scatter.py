...
def compute_minimum_distances(num_fishers):
    """ 
    num_fishers: number of Principal Components (which, for the FisherFaces recognizer,
        is equal to the number of classes minus one)
    num_fishers type: 
        Compute a leaving one (of each class) out 61 times and calculate distance to
    ...
    for k in range(61):
        ...
        projections = np.array(projections).reshape(len(projections), num_fishers)
...
vector_eucl, vector_mahal = compute_minimum_distances(len(class_numbers)-1)
...
for i in range(len(class_numbers)):
    ...
    utils.draw_circumference(ax, [0,0], r1, col1)
    utils.draw_circumference(ax, [0,0], r2, col2)
...
for i in range(len(class_numbers)):
    ...
    # Plot circumferences with average distances
