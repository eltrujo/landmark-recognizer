# -*- coding: utf-8 -*-
"""
Created on Wed Apr 18 17:42:45 2018
...
# Euclidean distance
fig = plt.figure(1)
plt.suptitle(video_name[:-4] + ' | Euclidean | ' + str(num_components) + ' components')

for i in range(len(model_names)):
    # Open json file and parse data to numpy array
    f = open(path + 'Videos/' + 'labels_' + model_names[i] + '_' + video_name[:-4] + '_2d_' + str(num_components) + '.json', 'r')
    data = json.load(f)
    f.close()
    num_frames = len(data)
...
fig = plt.figure(2)
plt.suptitle(video_name[:-4] + ' | Mahalanobis | ' + str(num_components) + ' components')
