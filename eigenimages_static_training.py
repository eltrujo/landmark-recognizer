# -*- coding: utf-8 -*-
"""
Created on Mon Feb 26 14:40:27 2018
...
# Define number of Principal Components for the PCA
num_components = 40
...
# Save some eigenimages
utils.save_eigenimages(eigen_imgs, height, path, amount=14)
...
for j in range(len(test_samples)):
    ...
    if j == len(test_samples)-1:
        ...
        for i in range(len(class_names)):
            plot_handles.append(plt.plot(x_values[60*i:(60*i+60)],dists[60*i:(60*i+60)], colors[i], label=class_names[i], linewidth=0.85)[0])
            min_dists.append(min(dists[60*i:(60*i+60)]))
    else:
        for i in range(len(class_names)):
            plt.plot(x_values[60*i:(60*i+60)],dists[60*i:(60*i+60)], colors[i], label=class_names[i][4:], linewidth=0.85)
            min_dists.append(min(dists[60*i:(60*i+60)]))
...
projections = np.array(projections).reshape(len(images), num_components)
