# -*- coding: utf-8 -*-
"""
Created on Wed May  9 14:06:07 2018
...
def compute_3_hists(img, hist):
    """
    img: Image with 3 channels RGB
    hist: np.zeros(768, dtype=np.int) because 768 = 256*3
    """
    hist = np.zeros(768, dtype=np.int)
    for i in range(3):
        hist[256*i:256*(i+1)] = cv2.calcHist([img], [i], None, [256], [0,256]).ravel()
    return hist
...
# Load images
class_names = os.listdir(path + 'Modified Images Reduced/')
class_numbers = list(map(lambda x: int(x[0]), class_names))
images, labels = utils.load_imagesRGB_and_labels(path + 'Modified Images Reduced/')
num_samples, height, width, _ = images.shape

# Compute histograms
hists = np.zeros((num_samples,768), dtype=np.int)
hist_zero = np.zeros(768, dtype=np.int)
for i, img in enumerate(images):
    hists[i] = compute_3_hists(img, hist_zero)
    
if model_name == 'eigen':
    num_components_array = np.arange(10, 151, 10)
...
for index, num_components in enumerate(num_components_array):
    ...
    for video_name in video_names:
        ...
        if im is not None:
            ...
            im_projected = np.dot(eigen_imgs.T, (hist - mean_vector).T).reshape(1,num_components)
